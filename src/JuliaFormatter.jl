...
function partial_formatting_blocks()
    # partial formatting
    regions = Tuple{Int,Int,String}[]
    for skip in s.doc.format_skips
        skip[4] || continue
        push!(regions, (skip[1], skip[2], skip[3]))
    end

    f = regions -> (n, s) -> begin
        (length(regions) == 0 || is_leaf(n)) && return
        if n.startline in regions[1][1]:regions[1][2]
            pushfirst!(n.nodes, Notcode(n.startline, n.startline))
        end
    end
end
...
function format_text(
    ...
    s = State(Document(text, lines), indent, margin, opts)
    t = pretty(style, x, s)
    ...
    # if length(regions) > 0
    # @info "" regions s.doc.format_skips lines
    # end
