...
Given a list of line ranges `lines`, return the inverted list of
line ranges.
"""
function invert_line_ranges(ranges)
    iranges = Tuple{Int,Int}[]
...
struct Document
    ...
    # the contents.
    format_skips::Vector{Tuple{Int,Int,String,Bool}}
end
...
function Document(text::AbstractString, partial_formats::Vector{Tuple{Int,Int}})
    ...
    comments = Dict{Int,Tuple{Int,String}}()
    semicolons = Set{Int}()
    format_skips = Tuple{Int,Int,String,Bool}[]
    ...
    for t in CSTParser.Tokenize.tokenize(text)
        ...
        elseif t.kind === Tokens.COMMENT
            ...
            elseif occursin(r"^#!\s*format\s*:\s*on\s*$", t.val) && length(stack) > 0
                ...
                idx2 = findlast(c -> c == '\n', str)
                str = str[idx1:idx2]
                push!(format_skips, (pop!(stack), t.startpos[1], str, false))
                str = ""
                format_on = true
            end
